...
/*  public Nodo(int n){
    ...
    public void grafico() {
        if (this.ver) {
            System.out.println("\t" + this.v + " [Nodo de cor = VERMELHO];");
        } else {
            System.out.println("\t" + this.v + " [Nodo de cor = PRETO];");
        }

        if (this.esq != Arvore.nil) {
            System.out.println("\t" + this.v + " -> " + this.esq.v + " [Filho = \" esq\"];");
            this.esq.grafico();
        }
        else {
            System.out.println("\t" + this.v + " -> [Sem filho = \" esq\"];");
        }

        if (this.dir != Arvore.nil) {
            System.out.println("\t" + this.v + " -> " + this.dir.v + " [Filho = \" dir\"];");
            this.dir.grafico();
        }
        else {
            System.out.println("\t" + this.v + " -> [Sem filho = \" dir\"];");
        }
    }
}
